...
module.exports = function eventRegistrationCompleteTemplate({ name, eventName, teamName, whatsappGroupLink, hasSubmission = true }) {
  const teamSection = teamName 
    ? `<p>Your team, <strong>${teamName}</strong>, is now fully registered.</p>` 
    : '';

  const mainParagraph = hasSubmission
    ? `We are excited to inform you that your submission for <strong>${eventName}</strong> has been reviewed and marked as completed.`
    : `We are excited to inform you that your registration for <strong>${eventName}</strong> is now confirmed and active.`;

  const whatsappSection = whatsappGroupLink 
    ? `
      <hr class="divider" />
      <p><strong>Join the official WhatsApp group for ${eventName}</strong> to receive announcements, schedule changes, and direct support:</p>
